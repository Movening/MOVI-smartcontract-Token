...
contract Movening is BEP20Detailed, BEP20 {
  ...
  constructor() BEP20Detailed("Movening", "MOVI", 18) {
    ...
    transferTax = 0;
    tradeCooldown = 15;
    marketingPool = 0x580AD98824A13947DBa3877C2Db1c6d188a7ff19; 
  }

   
 
  function addBlackList(address _wallet, bool _status) external onlyOwner {
    isBlacklist[_wallet]= _status;
    emit changeBlacklist(_wallet, _status);
  }  

  function setCooldownTrades(uint8 _tradeCooldown) external onlyOwner {
    tradeCooldown = _tradeCooldown;
    emit changeCooldown(_tradeCooldown);
  }

  function addLiquidityPool(address _lpAddress, bool _status) external onlyOwner {
    liquidityPool[_lpAddress] = _status;
    emit changeLiquidityPoolStatus(_lpAddress, _status);
  }

  function setRewardsPool(address _marketingPool) external onlyOwner {
  ...
  function addWhitelistTax(address _address, bool _status) external onlyOwner {
